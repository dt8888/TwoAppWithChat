...
+ (UIButton *)createButtonWithFrame:(CGRect)frame title:(NSString *)title textColor:(UIColor *)color  imageName:(NSString *)imageName target:(id)target action:(SEL)action isCorner:(BOOL)isCorner isDriction:(int)driction
{
    ...
    if(isCorner){
        UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:btn.bounds  byRoundingCorners:UIRectCornerTopRight |  UIRectCornerBottomRight    cornerRadii:CGSizeMake(25, 25)];
    ...
    }else if (driction==2)
    {
        ...
        btn.contentMode = UIViewContentModeScaleAspectFit;
        [btn setImageEdgeInsets:UIEdgeInsetsMake(0, 5, 0, 8)];
    }
...
//     textView.tintColor = greenColor;
//   // textView.backgroundColor = lightColor;
//    if (placeHolder) {
